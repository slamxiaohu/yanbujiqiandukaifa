...
# class DataThread(LoadStreams):
#
#     def __init__(self):
#         self.state = 1
#
#
#     def run(self, i, cap, stream, state):
#         # Read stream `i` frames in daemon thread
#         n, f = 0, self.frames[self.i]  # frame number, frame array
#         while True:
#             if self.state == 0:
#                 break
#             elif self.state == 1:
...
#                         else:
#                             LOGGER.warning('WARNING ⚠️ Video stream unresponsive, please check your IP camera connection.')
#                             self.imgs[self.i] = np.zeros_like(self.imgs[self.i])
#                             self.cap.open(self.stream)  # re-open stream if signal was lost
#                     time.sleep(0.0)  # wait time
#             elif self.state == 2:
#                 time.sleep(0.0)
#
